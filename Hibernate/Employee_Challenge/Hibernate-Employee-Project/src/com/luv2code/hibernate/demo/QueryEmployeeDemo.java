...
public class QueryEmployeeDemo {
	...
	public static void main(String[] args) {
		...
		try {
			...
			employees = session.createQuery("from Employee s where" + " s.lastName='Doe' OR s.firstName='Daffy'")
					.getResultList();
			System.out.println("Employees who have last name of Doe or firstName of Daffy");
			displayStudents(employees);

			// query using LIKE clause
			employees = session.createQuery("from Employee s where" + " s.email LIKE '%gmail.com'").getResultList();
